test_that("detrending works", {
  ...
  corrected10 <- img_detrend_boxcar(img, 10, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected10[, , 1, ])), 2), 1.27)
  corrected50 <- img_detrend_boxcar(img, 50, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected50[, , 1, ])), 2), 2.08)
  corrected100 <- img_detrend_boxcar(img, 100, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected100[, , 1, ])), 2), 3.04)
  corrected300 <- img_detrend_boxcar(img, 300, purpose = "ff")
  ...
  expect_equal(
    mean(
      purrr::map_dbl(
        corrected_autos,
        \(x) mean(brightness_pillars(x[, , 1, ]))
      )
    ),
    1.47,
    tolerance = 0.1
  )
  corrected10 <- img_detrend_exp(img, 10, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected10[, , 1, ])), 2), 1.29)
  corrected50 <- img_detrend_exp(img, 50, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected50[, , 1, ])), 2), 2.38)
  corrected100 <- img_detrend_exp(img, 100, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected100[, , 1, ])), 2), 3.38)
  corrected1000 <- img_detrend_exp(img, 1000, purpose = "ff")
  ...
  expect_warning(
    img_detrend_polynom(img, "auto", purpose = "ff"),
    "polynomial degree"
  )
  expect_equal(
    ijtiff::as_ijtiff_img(img_detrend_degree_specified(img, NA, "ff")),
  ...
  corrected2 <- img_detrend_polynom(img, 2, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected2[, , 1, ]),
    na.rm = TRUE
  ), 1), 3.5)
  corrected4 <- img_detrend_polynom(img, 4, purpose = "ff")
  expect_equal(round(mean(brightness_pillars(corrected4[, , 1, ]),
    na.rm = TRUE
  ), 1), 2.6)
  corrected8 <- img_detrend_polynom(img, 8, purpose = "ff")
