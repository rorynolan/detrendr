...
test_that("detrending entire derectories works", {
  ...
  if (get_os() == "mac") {
    expect_equal(dir("detrended"),
                 paste0(c("2ch_ij_detrended_thresh=Triangle=0.6,Triangle=0.6_",
                          "bleached_detrended_thresh=Triangle=41.622_"),
                        c("boxcar_for_FFS_l=auto=NA,auto=2.tif",
  ...
  if (get_os() == "mac") {
    expect_equal(dir("detrended"),
                 paste0(c("2ch_ij_detrended_thresh=Triangle=0.6,Triangle=0.6_",
                          "bleached_detrended_thresh=Triangle=41.622_"),
                          "exponential_",
                        c("for_FFS_tau=auto=NA,auto=6.1767578125.tif",
  ...
  detrendeds <- purrr::map(orig_imgs, autothresholdr::mean_stack_thresh,
                           method = "tri") %>%
    purrr::map(~ suppressWarnings(img_detrend_polynom(., degree = 2,
                                                      purpose = "ff")))
  set.seed(1)
  suppressWarnings(dir_detrend_polynom(degree = 2, thresh = "tri",
                                       purpose = "ff", msg = FALSE))
  detrendeds_dir <- dir(pattern = "detrended.*tif", recursive = TRUE) %>%
    purrr::map(ijtiff::read_tif, msg = FALSE)
  expect_equal(purrr::map(detrendeds, dim), purrr::map(detrendeds_dir, dim))
  ...
  expect_equal(dir("detrended"),
               paste0(c("2ch_ij_detrended_thresh=Triangle=0.6,Triangle=0.6_",
                        "bleached_detrended_thresh=Triangle=41.622_"),
                      c("polynomial_for_FFS_degree=2,2.tif",
