...
test_that("detrending works", {
  ...
  expect_true(round(mean(brightness_pillars(corrected)), 1) %in% c(1.9))
  ...
  expect_equal(round(mean(brightness_pillars(corrected100)), 1), 3.3)
  ...
  expect_equal(round(mean(brightness_pillars(corrected100)), 1), 4)
  ...
  expect_true(round(mean(brightness_pillars(corrected2), na.rm = TRUE), 1) %in%
                c(seq(3.5, 3.9, 0.1)))
  corrected4 <- img_detrend_polynom(img, 4, seed = 0, parallel = 2)
  expect_true(round(mean(brightness_pillars(corrected4), na.rm = TRUE), 1) %in%
                 c(2.7, 2.8))
